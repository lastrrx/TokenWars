...
function showPageContentWithTimeout(pageName) {
    const timeout = setTimeout(() => {
        // Force hide loading state after 5 seconds
        hideSkeletonScreen(pageName);
        const loadingStates = document.querySelectorAll('.loading-state, .loading');
...
// Update loadPageContentAsync to include timeout
async function loadPageContentAsync(pageName) {
    // Prevent duplicate loading
    if (pageStates[pageName].loading) {
        console.log(`⏳ ${pageName} already loading...`);
    ...
    const loadingTimeout = showPageContentWithTimeout(pageName);
