...
/**
 * Render Pair Analytics Table with Enhanced Diagnostics - UPDATED
 */
async function renderPairAnalyticsTableWithDiagnostics() {
    try {
        ...
        if (pairs.length === 0) {
            tbody.innerHTML = `
                <tr>
                    <td colspan="7" style="text-align: center; padding: 2rem; color: #94a3b8;">
                        No token pairs found in database.
                        <br><br>
                        <div style="font-size: 0.9em; color: #6b7280; margin-top: 1rem;">
                            <div>Pairs are automatically generated by the edge function every 5 minutes.</div>
                            <div style="margin-top: 0.5rem;">Check back in a few minutes or ensure the edge function is running.</div>
                        </div>
                        <br>
                        <button onclick="loadAllTokenPairsWithDiagnostics()" style="
                            padding: 0.5rem 1rem;
                            background: #22c55e;
                            color: white;
                            border: none;
            ...
            `;
            debugLog('pairsTable', 'No pairs to display - showing empty state with refresh option');
            return;
        }
...
async function createCompetitionWithSmartContract(config) {
    try {
        ...
        console.log('🆔 Generated competition ID:', competitionId);
